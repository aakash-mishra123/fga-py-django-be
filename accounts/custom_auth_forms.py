    ...
    def get_context(self, name, value, attrs):
        ...
        if not value or value.startswith(UNUSABLE_PASSWORD_PREFIX):
            ...
            try:
                ...
                summary.extend([
                    {"label": gettext(key), "value": val}
                    for key, val in hasher.safe_summary(value).items()
                ])
            except ValueError:
                summary.append({
...
class PasswordResetForm(forms.Form):
    ...
    def get_users(self, email):
        ...
        return (
            ...
            _unicode_ci_compare(email, getattr(user, email_field_name))
        )

    def save(
        self, domain_override=None, subject_template_name="registration/password_reset_subject.txt",
