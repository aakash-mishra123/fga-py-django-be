...
from product.models import Product, CatalogCategory,CatalogSubCategory, Images, Seo, ProductBrand, ProductTag
...
class ProductBrandAdmin(admin.ModelAdmin):
    def has_view_permission(self, request, obj=None):
        ...
        desired_permission_codename = 'view_productbrand'
    ...
    def has_change_permission(self, request, obj=None, extra_context=None):
        user_group_permissions = get_group_permissions(request.user)
        desired_permission_codename = 'change_productbrand'
        if any(perm.codename == desired_permission_codename for perm in user_group_permissions):
            return True
        else:
            return False
    ...
    def has_add_permission(self, request, obj=None):
        user_group_permissions = get_group_permissions(request.user)
        desired_permission_codename = 'add_productbrand'
        if any(perm.codename == desired_permission_codename for perm in user_group_permissions):
            return True
        else:
            return False
    ...
    def has_delete_permission(self, request, obj=None):
        user_group_permissions = get_group_permissions(request.user)
        desired_permission_codename = 'delete_productbrand'
        if any(perm.codename == desired_permission_codename for perm in user_group_permissions):
            return False
        else:
            return False
    ...
    def export_as_csv(modeladmin, request, queryset):
        ...
        for product_brand in queryset:
            ...
            writer.writerow([image_url, product_brand.name, product_brand.slug, product_brand.status])  # Add more fields
    ...
    actions = ['export_as_csv', 'disassociate_products']  # Add a new action
    ...
    def disassociate_products(modeladmin, request, queryset):
        for product_brand in queryset:
            ...
            if associated_products.exists():
                messages.error(
                    request,
                    f"Cannot delete '{product_brand.name}' because it's associated with products. Please update those products first.",
                )
            else:
...
class CatalogCategoryAdmin(admin.ModelAdmin):
    def has_view_permission(self, request, obj=None):
        user_group_permissions = get_group_permissions(request.user)
        desired_permission_codename = 'view_catalogcategory'
        if any(perm.codename == desired_permission_codename for perm in user_group_permissions):
            return True
        else:
            return False
    ...
    def has_change_permission(self, request, obj=None):
        user_group_permissions = get_group_permissions(request.user)
        desired_permission_codename = 'change_catalogcategory'
        if any(perm.codename == desired_permission_codename for perm in user_group_permissions):
            return True
        else:
            return False
    ...
    def has_add_permission(self, request, obj=None):
        user_group_permissions = get_group_permissions(request.user)
        desired_permission_codename = 'add_catalogcategory'
        if any(perm.codename == desired_permission_codename for perm in user_group_permissions):
            return True
        else:
            return False
    ...
    def has_delete_permission(self, request, obj=None):
        user_group_permissions = get_group_permissions(request.user)
        desired_permission_codename = 'delete_catalogcategory'
        if any(perm.codename == desired_permission_codename for perm in user_group_permissions):
            return False
        else:
            return False
...
class CatalogSubCategoryAdmin(admin.ModelAdmin):
    def has_view_permission(self, request, obj=None):
        user_group_permissions = get_group_permissions(request.user)
        desired_permission_codename = 'view_catalogsubcategory'
        if any(perm.codename == desired_permission_codename for perm in user_group_permissions):
            return True
        else:
            return False
    ...
    def has_change_permission(self, request, obj=None):
        user_group_permissions = get_group_permissions(request.user)
        desired_permission_codename = 'change_catalogsubcategory'
        if any(perm.codename == desired_permission_codename for perm in user_group_permissions):
            return True
        else:
            return False
    ...
    def has_add_permission(self, request, obj=None):
        user_group_permissions = get_group_permissions(request.user)
        desired_permission_codename = 'add_catalogsubcategory'
        if any(perm.codename == desired_permission_codename for perm in user_group_permissions):
            return True
        else:
            return False
    ...
    def has_delete_permission(self, request, obj=None):
        user_group_permissions = get_group_permissions(request.user)
        desired_permission_codename = 'delete_catalogsubcategory'
        if any(perm.codename == desired_permission_codename for perm in user_group_permissions):
            return False
        else:
            return False
    ...
    def upload_csv(self, request):
        if request.method == "POST":
            ...
            #     messages.error(request, 'Failed to decode the CSV file. Please ensure it is in UTF-8 encoding.')
            #     return HttpResponseRedirect(request.path_info)
...
class ProductAdmin(admin.ModelAdmin):
    def has_view_permission(self, request, obj=None):
        user_group_permissions = get_group_permissions(request.user)
        desired_permission_codename = 'view_prouct'
        if any(perm.codename == desired_permission_codename for perm in user_group_permissions):
            return True
        else:
            return False
    ...
    def has_change_permission(self, request, obj=None):
        user_group_permissions = get_group_permissions(request.user)
        desired_permission_codename = 'change_product'
        if any(perm.codename == desired_permission_codename for perm in user_group_permissions):
            return True
        else:
            return False
    ...
    def has_delete_permission(self, request, obj=None):
        user_group_permissions = get_group_permissions(request.user)
        desired_permission_codename = 'delete_product'
        if any(perm.codename == desired_permission_codename for perm in user_group_permissions):
            return True
        else:
            return False
    ...
    list_display = ('image_preview','product_name', 'category','subcategory','Brand', 'sku','price','status','edit_item')
    readonly_fields = ['image_preview']

    inlines = [ProductAdminInline, ProductAdminInlineSeo]

    fieldsets = [
        (None, {"fields": ['product_name', 'category','subcategory','Brand', 'sku','product_tags', 'price','product_description','product_short_description','product_weight','product_image','image_preview','status']}),
        ]
        ...
        if request.method == "POST":
            ...
            for fields in csv_reader:
                ...
                if len(fields) >= 14:
                    ...
                    print(Brand_name,'780')
                    print(product_name,"rawat")
                    ...
                    try:
                        ...
                        # else:
                        #     print(f"Failed to download image from URL: {product_image}")
                        #     default_image_path = 'path/to/your/default/image.jpg'  # Replace with your default image path
                        #     with open(default_image_path, 'rb') as default_image_file:
                        #         product_instance.product_image.save('default_image.jpg', ContentFile(default_image_file.read()))
                        ...
                        print(f"Brand '{Brand_name}' Brand does not exist.")
                        continue
    ...
    #                             CatalogCategory_instance = CatalogCategory.objects.get(name=category_name)
    #                             CatalogSubCategory_instance = CatalogSubCategory.objects.get(name=subcategory_name)
    #                             # print(CatalogSubCategory_instance)
    #                             ProductBrand_instance = ProductBrand.objects.get(name=Brand_name)


    #                             # Update the fields
    #                             product_instance.product_name = fields[0]
    #                             product_instance.name_slug = fields[1].strip()
    #                             product_instance.category = CatalogCategory_instance
    #                             product_instance.subcategory = CatalogSubCategory_instance
    #                             product_instance.Brand = ProductBrand_instance
    #                             product_instance.price = fields[6].strip()
    #                             product_instance.product_description = fields[8]
    #                             product_instance.product_short_description = fields[9]
    #                             product_instance.quantity = fields[10].strip()
    #                             product_instance.product_weight = fields[11].strip()
    #                             product_instance.product_image = fields[12].strip()
    #                             product_instance.status = fields[13].strip()

    #                             product_tags_instances = [ProductTag.objects.get_or_create(name=tag)[0] for tag in product_tags_names]
    #                             product_instance.product_tags.set(product_tags_instances)
    #                             products_to_update.append(product_instance)
    #                             print(product_instance,'Update')
    ...
    #                         try:
    #                             # If the product does not exist, create a new instance
    #                             CatalogCategory_instance = CatalogCategory.objects.get(name=category_name)
    #                             CatalogSubCategory_instance = CatalogSubCategory.objects.get(name=subcategory_name)
    #                             print(CatalogSubCategory_instance)
    #                             ProductBrand_instance = ProductBrand.objects.get(name=Brand_name)
    ...
    #                             )
    #                             # if fields[7].strip():
    #                             #     for tag in product_tags_names:
    #                             #         product_tags_instances, created = ProductTag.objects.get_or_create(name=tag)
    #                             #         print(product_tags_instances.id,'dhkjdkfhdhfkj')

    #                             #         product_instance.product_tags.set(product_tags_instances.id)
    #                             #         products_to_create.append(product_instance)
    #                             # else:
    #                             #     product_instance.product_tags.set(6)
    #                             #     products_to_create.append(product_instance)
    ...
    #                                 if default_image_response.status_code == 200:
    #                                     default_image_content = ContentFile(default_image_response.content)
    #                                     product_instance.product_image.set('no-image.png', default_image_content)
    #                                 else:
    #                                     print(f"Failed to download default image from URL: {default_image_url}")

    #                             else:
    #                                 # Download and save the image
    #                                 image_response = requests.get(fields[12])
    #                                 if image_response.status_code == 200:
    #                                     image_filename = fields[12].split('/')[-1]
    #                                     image_content = ContentFile(image_response.content)
    #                                     product_instance.product_image.set(image_filename, image_content)
    #                                 else:
    #                                     print(f"Failed to download image from URL: {fields[12]}")
    #                         except ObjectDoesNotExist as e:
    #                             print(f"Object does not exist: {e}")
    #             # Bulk create new instances
    #             print(products_to_create,'Save')
    #             Product.objects.bulk_create(products_to_create)

    #             # # Save the new instances to get primary keys
    #             # for product_instance in products_to_create:
    #             #     product_instance.save()

    #             # Bulk update existing instances
    #             print(products_to_update,'Update')
...
class ProductTagAdmin(admin.ModelAdmin):
    def has_view_permission(self, request, obj=None):
        user_group_permissions = get_group_permissions(request.user)
        desired_permission_codename = 'view_producttag'
        if any(perm.codename == desired_permission_codename for perm in user_group_permissions):
            return True
        else:
            return False
    ...
    def has_change_permission(self, request, obj=None):
        user_group_permissions = get_group_permissions(request.user)
        desired_permission_codename = 'change_producttag'
        if any(perm.codename == desired_permission_codename for perm in user_group_permissions):
            return True
        else:
            return False
    ...
    def has_add_permission(self, request, obj=None):
        user_group_permissions = get_group_permissions(request.user)
        desired_permission_codename = 'add_producttag'
        if any(perm.codename == desired_permission_codename for perm in user_group_permissions):
            return True
        else:
            return False
    ...
    list_display = ('name', 'slug', 'status','edit_item')
    ...
    def upload_csv_product_tag(self, request):
        if request.method == "POST":
            ...
            url = reverse('admin:product_view_producttag_changelist')
            return HttpResponseRedirect(url)
