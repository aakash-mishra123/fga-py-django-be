...
class CaptchaTextInput(BaseCaptchaTextInput):
    ...
    def render(self, name, value, attrs=None, renderer=None):
        ...
        rendered_widgets=  super().render(name, self._value, attrs=attrs, **extra_kwargs)

        # Include error messages if there are any
        error_messages = self.get_context(name, value, attrs).get("error_messages", [])
        error_html = '<div class="captcha-error">%s</div>' % ', '.join(error_messages)

        return rendered_widgets + error_html
