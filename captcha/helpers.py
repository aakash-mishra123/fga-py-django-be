...
def unicode_challenge():
    """
    Generate a CAPTCHA challenge using Unicode characters with accents.
    
    Returns:
        tuple: (uppercase challenge, lowercase challenge)
    """
    chars = "äàáëéèïíîöóòüúù"
...
def word_challenge():
    """
    Generate a CAPTCHA challenge using words from a dictionary file.
    ...
    while True:
        ...
        if (word_length >= settings.CAPTCHA_DICTIONARY_MIN_LENGTH and 
            word_length <= settings.CAPTCHA_DICTIONARY_MAX_LENGTH):
            break
...
def huge_words_and_punctuation_challenge():
    """
    Generate a CAPTCHA challenge using two words joined by punctuation.
    
    Used to test longer CAPTCHA challenges with punctuation.
    Originally created for Issue 39: http://code.google.com/p/django-simple-captcha/issues/detail?id=39
